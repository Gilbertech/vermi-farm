...
import { PortfolioService, PortfolioTransaction } from '../services/portfolioService';
...
const Portfolio: React.FC = () => {
  const { stats, loans, loading: appLoading, error: appError } = useApp();
  ...
  // Mock portfolio transaction data for fallback
  const mockPortfolioTransactions: Record<string, PortfolioTransaction[]> = {
    loan: [
      {
        id: '1',
        tx_code: 'LN001234567',
    ...
    revenue: [
      {
        id: '2',
        tx_code: 'RV001234568',
        portfolio_type: 'revenue',
        from_account: 'Interest Collection',
        to_account: 'Revenue Portfolio',
        amount: 2500,
        fees: 15,
        status: 'completed',
  ...
  // Load portfolio transactions based on active tab
  useEffect(() => {
    const loadPortfolioTransactions = async () => {
      try {
        ...
        try {
          const response = await PortfolioService.getPortfolioTransactions(activeTab, { limit: 1000 });
          setPortfolioTransactions(response.items);
        } catch (apiError) {
          console.warn(`API ${activeTab} portfolio failed, using mock data:`, apiError);
          setPortfolioTransactions(mockPortfolioTransactions[activeTab] || []);
        }
