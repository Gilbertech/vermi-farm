...
interface UpdateInterestLimitFormProps {
  group: any;
  onClose: () => void;
}

const UpdateInterestLimitForm: React.FC<UpdateInterestLimitFormProps> = ({ group, onClose }) => {
  const { updateGroupInterestRate } = useApp();
  const [interestRate, setInterestRate] = useState(group.interestRate || 5);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateGroupInterestRate(group.id, interestRate);
...
export default UpdateInterestLimitForm;